...
import { CONTRACT_TYPES, KYC_STATUSES, MOCK_COUNTRIES, MOCK_TIMEZONES, STATUS_LABELS, VOLUME_TIERS } from '../types/account';
...
export default function MerchantAccountsPage() {
  ...
  const fallbackAccountUUID = searchParams.get('account_uuid') || '';
  ...
  useEffect(() => {
    const accountUUID = selectedAccountUUID || fallbackAccountUUID;
    if (!accountUUID) {
      setLegalEntities([]);
      return;
    }
    listLegalEntities({ account_uuid: accountUUID, page: 1, page_size: 300 })
      .then((data) => setLegalEntities(data.legal_entities || []))
      .catch(() => setLegalEntities([]));
  }, [selectedAccountUUID, fallbackAccountUUID]);
  ...
  const resolvedAccountUUID = selectedLegalEntity?.account_uuid || fallbackAccountUUID;

  useEffect(() => {
    if (!selectedMerchant || !selectedLegalEntity) return;
    const params = new URLSearchParams(searchParams);
    if (selectedLegalEntity.account_uuid) params.set('account_uuid', selectedLegalEntity.account_uuid);
  ...
  const handleSave = async () => {
    if (!selectedMerchant || !resolvedLegalEntityUUID || !resolvedAccountUUID) {
      addToast('Merchant, legal entity, and account are required', 'error');
      return;
    }
        ...
        <form className="filters-form" onSubmit={(e) => { e.preventDefault(); fetchItems(); }}>
          ...
          <div className="filter-group">
            <select
              ...
              disabled={!selectedAccountUUID && !fallbackAccountUUID}
            >
